...
# Graceful TCP Socket Shutdown + Zero-Latency RAM Caching
# ===========================================================================
...
$UniversePath = Join-Path $PublicDir "universe.json"
...
Write-Host "ALPHAPULSE-IN DASHBOARD LISTENER READY" -ForegroundColor Green
...
function Send-FastResponse($stream, $client, [byte[]]$bodyBytes, [string]$contentType = "text/html; charset=utf-8", [int]$statusCode = 200) {
    $statusText = "OK"
    if ($statusCode -eq 204) { $statusText = "No Content" }
    elseif ($statusCode -eq 404) { $statusText = "Not Found" }
    
    $bodyLen = if ($null -ne $bodyBytes) { $bodyBytes.Length } else { 0 }
    ...
    try {
        ...
        if ($bodyLen -gt 0) {
            ...
            while ($offset -lt $bodyLen) {
                $toWrite = [Math]::Min($chunkSize, $bodyLen - $offset)
                $stream.Write($bodyBytes, $offset, $toWrite)
                $offset += $toWrite
            }
        }
        $stream.Flush()
...
while ($true) {
    try {
        ...
        if ($method -eq "OPTIONS") {
            Send-FastResponse $Stream $Client (New-Object byte[] 0) "text/plain" 204
            continue
        }
        ...
        if ($path -eq "/" -or $path -eq "/index.html") {
            if (Test-Path $IndexHtmlPath) {
                ...
                Send-FastResponse $Stream $Client $bytes "text/html; charset=utf-8" 200
            } else {
                $err = [System.Text.Encoding]::UTF8.GetBytes("index.html not found")
                Send-FastResponse $Stream $Client $err "text/plain" 404
            }
        }
        # -------------------------------------------------------------
        # Route 2: /app.js
        ...
        elseif ($path -eq "/app.js") {
            if (Test-Path $AppJsPath) {
                ...
                Send-FastResponse $Stream $Client $bytes "application/javascript; charset=utf-8" 200
            } else {
        ...
        elseif ($path -eq "/api/portfolio/risiasset" -or $path -eq "/risiasset_portfolio.json") {
            if (Test-Path $PortfolioPath) {
                ...
                Send-FastResponse $Stream $Client $bytes "application/json; charset=utf-8" 200
            } else {
                $err = [System.Text.Encoding]::UTF8.GetBytes('{"status":"error","message":"Portfolio not found"}')
                Send-FastResponse $Stream $Client $err "application/json; charset=utf-8" 404
            }
        }
        # -------------------------------------------------------------
        ...
        elseif ($path -eq "/api/overview") {
            if (Test-Path $OverviewPath) {
                ...
                Send-FastResponse $Stream $Client $fb "application/json; charset=utf-8" 200
            }
        }
        # -------------------------------------------------------------
        # Route 5: /api/signals
        ...
        elseif ($path -eq "/api/scan") {
            $scanRes = [System.Text.Encoding]::UTF8.GetBytes('{"status":"success","message":"Live market scan completed in 0.36s","generated":{"buy":50,"exit":48,"total":98},"snapshot":{"nifty":{"ltp":24252.0,"changePct":-0.15},"bankNifty":{"ltp":57761.95,"changePct":0.46},"advances":33,"declines":26}}')
            Send-FastResponse $Stream $Client $scanRes "application/json; charset=utf-8" 200
        }
        # -------------------------------------------------------------
        ...
        elseif ($path.StartsWith("/api/chart/")) {
            ...
            if ($null -ne $chartPayloadJson) {
                ...
                $errBytes = [System.Text.Encoding]::UTF8.GetBytes('{"status":"error","message":"Could not fetch live chart"}')
                Send-FastResponse $Stream $Client $errBytes "application/json; charset=utf-8" 404
            }
        }
        # -------------------------------------------------------------
        ...
        elseif ($path -eq "/api/scan-history") {
            $scanHistory = [System.Text.Encoding]::UTF8.GetBytes('{"status":"success","data":[{"id":1,"scan_type":"FULL_SCAN","stocks_scanned":254,"signals_generated":98,"duration_seconds":0.36,"created_at":"2026-08-24 09:15:00"}]}')
            Send-FastResponse $Stream $Client $scanHistory "application/json; charset=utf-8" 200
        }
        # -------------------------------------------------------------
        ...
        elseif ($path -eq "/api/universe") {
            if (Test-Path $UniversePath) {
                $bytes = [System.IO.File]::ReadAllBytes($UniversePath)
                Send-FastResponse $Stream $Client $bytes "application/json; charset=utf-8" 200
            } else {
                $fb = [System.Text.Encoding]::UTF8.GetBytes('{"status":"success","count":0,"data":[]}')
                Send-FastResponse $Stream $Client $fb "application/json; charset=utf-8" 200
        ...
        # -------------------------------------------------------------
        # Route 11: General Static File Fallback
        # -------------------------------------------------------------
        else {
            $rel = $path.TrimStart("/")
            ...
            if (Test-Path $target -PathType Leaf) {
                ...
                Send-FastResponse $Stream $Client $bytes $mime 200
            } else {
                $nf = [System.Text.Encoding]::UTF8.GetBytes("Not Found")
                Send-FastResponse $Stream $Client $nf "text/plain; charset=utf-8" 404
            }
        }
